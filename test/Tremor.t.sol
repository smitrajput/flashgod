...
contract TremorTest is Test {
    ...
    function test_dominoeFlashLoans_ethereum() public {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            uint256 fee = (amounts[i] * 1000) / 1000000;
        ...
        for (uint256 i = 0; i < uniPools.length; i++) {
            ...
            IUniswapV3Pool uniPool = IUniswapV3Pool(
                PoolAddress.computeAddress(
                    addresses.provider.UNI_V3_FACTORY, PoolAddress.PoolKey({token0: token0, token1: token1, fee: fee})
                )
            );
            deal(
                token0,
    ...
    //     for (uint256 i = 0; i < assets.length; i++) {
    //         // TODO: fix. 0.055% coz 0.05% fails
    //         uint256 fee = (amounts[i] * 550) / 1000000;
    //         deal(assets[i], address(tremor), fee);
    //     }
    ...
    //     assets[4] = addresses.RETH;
    //     assets[5] = addresses.LINK;
    //     assets[6] = addresses.USDT;
    ...
    //     uniPools[1] = abi.encode(addresses.WBTC, addresses.WETH, uint16(3000));
    //     uniPools[2] = abi.encode(addresses.USDC, addresses.WETH, uint16(500));
    ...
    //     assets[0] = addresses.WETH;
    //     assets[1] = addresses.USDC;
    //     assets[2] = addresses.WSTETH;
    //     assets[3] = addresses.RETH;
    //     assets[4] = addresses.LINK;
    //     assets[5] = addresses.USDT;
    ...
    //     bytes[] memory uniPools = new bytes[](3);
    //     uniPools[0] = abi.encode(addresses.WETH, addresses.USDC, uint16(500));
    //     uniPools[1] = abi.encode(addresses.WSTETH, addresses.WETH, uint16(3000));
    //     uniPools[2] = abi.encode(addresses.USDC, addresses.WETH, uint16(500));
