...
contract PairFlash is IUniswapV3FlashCallback, PeripheryPayments, Test {
    ...
    function initFlash(FlashParams memory params) external {
        ...
        console.log(
            ...
            IERC20(params.token0).balanceOf(address(this)) / (10 ** IERC20Metadata(params.token0).decimals())
        );

        console.log(
            IERC20Metadata(params.token1).symbol(),
            IERC20(params.token1).balanceOf(address(this)) / (10 ** IERC20Metadata(params.token1).decimals())
        );
    }
    ...
    function uniswapV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
        ...
        // simulate minting tokens to this contract using deal
        deal(token0, address(this), fee0);
        deal(token1, address(this), fee1);
